...
class Rebuilder extends Command
{
    ...
    /**
     * Truncate mode key value
     */
    const MODE_TRUNCATE = 'truncate';

    /**
     * Delete mode key value
     */
    ...
    /**
     * Rebuild mode: truncate or delete
     *
     * @var string
     */
    protected $mode = self::MODE_DELETE;
    ...
    protected function deleteAutogeneratedUrlRewrites()
    {
        $this->output->write('[STEP 1] Truncating table `catalog_url_rewrite_product_category` '
            . 'and deleting auto-generated `url_rewrite`... ');
        ...
        $connection->delete($moduleResource->getTable('url_rewrite'), 'is_autogenerated = 1');
        $connection->query('set foreign_key_checks = 1');
    ...
    protected function checkProductUrlKeys()
    {
        ...
        $select->from(
            ...
            array(
                'url_key' => 'value',
                'count' => "COUNT(`cpev`.`{$this->idColumn}`)",
            )
        )->join(
            array('cpe' => $moduleResource->getTable('catalog_product_entity')),
            "cpe.{$this->idColumn} = cpev.{$this->idColumn}",
            ''
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption(self::INPUT_KEY_MODE)) {
            $this->mode = $input->getOption(self::INPUT_KEY_MODE);
        }
        ...
        try {
            ...
            $this->init();
            if (preg_match('/' . self::MODE_TRUNCATE . '/', $this->mode)) {
                $this->truncateTables();
            } else {
                $this->deleteAutogeneratedUrlRewrites();
            }
